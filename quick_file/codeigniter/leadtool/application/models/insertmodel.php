<?php
    class Insertmodel extends CI_Model {
		...
		if(1<=$word_count && $word_count<=$message_value[0]){
		$start=1;
		$end=$message_value[0];
		...
		$score_comments=$row->score;	
		}
		
		$start1=$message_value[$i];
		$start=$start1+1;
		$end=$message_value[$j];
		...
		$sample_score=$this->input->post('needsample');
		...
		$this->db->where('score', $this->input->post('country'));
		$query = $this->db->get('country'); //get all data from user_profiles table that belong to the respective user
		$row=$query->row(); // the data
		$country_name=$row->name;
		$message=$this->input->post('comments');
		$message_url=preg_replace('/\s+/', '', $message);
		
		$key="AIzaSyAyWQYSko3EH_GgO4gOcPgK5jg-CqMai9g";
		$url = "https://www.googleapis.com/language/translate/v2?key=$key&target=de&q=$message_url";
		...
		$this->db->set('country', $country_name);
		...
		$this->db->set('score_country', $country_score);
		$this->db->set('score_companyname', $score_companyname);
		$this->db->set('score_businesstype', $this->input->post('businesstype'));
		$this->db->set('score_contactinformation', $score_contactinformation);
		$this->db->set('score_enquiredescription', $score_comments);
		$this->db->set('score_unitprice', $score_unitprice);
		$this->db->set('score_sample', $sample_score);
		$this->db->set('score_attachedment', $score_attachedment);
		$this->db->set('score_country', $this->input->post('country'));
		$this->db->set('score_industry', $this->input->post('industry'));
