<body>
	...
	<div class="form-group">
		...
		<?php
		foreach($country->result() as $con)
		{
		?>
			<option value="<?php echo $con->score;?>" class="form-paddding"><?php echo ucfirst($con->name);?> </option>
	...
	<label for="exampleInputname">Select Industry</label>
	  ...
	  <?php
		foreach($getindustry->result() as $getind)
		{
		?>
      ...
      <?php
		...
		{
		?>
			<option value="<?php echo $ind->score;?>" class="form-paddding" <?php if($getind->name) echo 'selected'?>><?php echo ucfirst($ind->name);?> </option>
