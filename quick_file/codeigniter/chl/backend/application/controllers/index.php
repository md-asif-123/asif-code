...
class index extends CI_Controller {
  ...
  function __construct() {
	...
	$this->load->model('insertmodel');
	 $this->load->database();$this->load->model('insertmodel');
	 $this->load->database();
	}
    ...
    $this->load->view("pages/listadmin", $data);
			...
			
		{
			...
			$this->load->view("pages/nolistadmin");
		}
	...
	for($i=0; $i<$cnt; $i++){
		...
		$id1=$id[$i];
		$data=$this->db->query("SELECT * FROM products WHERE id=$id1");

		foreach($data->result() as $row)
		{
			echo $row->title;?>&nbsp&nbsp&nbsp&nbsp
			<?php
			$img = $row->image;?>
			<?php if($img=='N'){?>
			...
			<img src="<?php echo base_url(); ?>uploads/<?php echo $row->image;?>" height="40" width="60"/>
		   <?php }?><br><br>
		   <?php
		}
